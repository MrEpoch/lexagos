...
test("Page", async () => {
  ...
  const allProducts = within(heroParallax).getAllByRole(
  ...
  allProducts.forEach((product) => {
    expect(
      within(product).getByRole("hero-parallax-product-image"),
    ).toBeDefined();
    expect(
      within(product).getByRole("hero-parallax-product-title"),
    ).toBeDefined();
  });
